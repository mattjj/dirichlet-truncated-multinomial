...
def kde(sigma,locs,vals=None):
    ...
    chunksize = 1000 # max intermediate array size is len(x)*chunksize doubles
    locchunks = np.array_split(locs,locs.shape[0]//chunksize+1)
    valchunks = np.array_split(vals,vals.shape[0]//chunksize+1)
    numchunks = len(locchunks)

    def p(x):
        assert x.ndim == 2
        # written with weird inner function f because ipython parallel can't
        # pickle function objects with closures
        def f((ls,vs,x,K,sigma)):
            return np.dot(np.ones(len(ls)),vs[:,na]
                    / (2*np.pi)**(K/2) / np.sqrt(sigma**K)
                    * np.exp(-0.5*((ls[:,na,:] - x[na,:,:])**2).sum(2)/sigma))
        return reduce(operator.add, dv.map_sync(f, zip(locchunks,valchunks,[x]*numchunks,[K]*numchunks,[sigma]*numchunks)))
